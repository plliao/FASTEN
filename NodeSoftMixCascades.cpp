...
int main(int argc, char* argv[]) {
  ...
  nodeSoftMixCascades.SetInitAlpha(InitAlpha);
  nodeSoftMixCascades.SetRegularizer(Regularizer);
  nodeSoftMixCascades.SetMu(Mu);
  nodeSoftMixCascades.SetWindow(Window);
  nodeSoftMixCascades.SetObservedWindow(observedWindow);
  nodeSoftMixCascades.SetAging(Aging);

  // load cascades from file
  nodeSoftMixCascades.LoadCascadesTxt(InFNm);
  ...
  if (MaxTimeStr.EqI("-1")) {
    ...
    for (int i=0; i<nodeSoftMixCascades.GetCascs(); i++) {
      if (nodeSoftMixCascades.CascH[i].GetMaxTm() > MaxTime) {
        MaxTime = nodeSoftMixCascades.CascH[i].GetMaxTm();
      }
    }
  } else {
  ...
  if (MinTimeStr.EqI("-1")) {
    ...
    MinTime = TFlt::Mx;
    for (int i=0; i<nodeSoftMixCascades.GetCascs(); i++) {
      if (nodeSoftMixCascades.CascH[i].GetMinTm() < MinTime && nodeSoftMixCascades.CascH[i].GetMinTm()!=0) {
        MinTime = nodeSoftMixCascades.CascH[i].GetMinTm();
      }
    }
  } else {
  ...
  switch (RunningMode) {
      ...
      case INFECTION_STEP:
        ...
        for (int i=0; i<nodeSoftMixCascades.GetCascs(); i++) {
          for (int j=0; j<nodeSoftMixCascades.CascH[i].Len() && nodeSoftMixCascades.CascH[i].NIdHitH[j].Tm < MaxTime; j++) {
            InfectionsV.Add(nodeSoftMixCascades.CascH[i].NIdHitH[j].Tm);
          }
        }
      ...
      case CASCADE_STEP:
        for (int i=0; i<nodeSoftMixCascades.GetCascs(); i++) {
          if (nodeSoftMixCascades.CascH[i].GetMaxTm()<MinTime+MaxTime) { Steps.Add(nodeSoftMixCascades.CascH[i].GetMaxTm()); }
        }
  ...
  nodeSoftMixCascades.Init();
  nodeSoftMixCascades.Infer(Steps, OutFNm);
  nodeSoftMixCascades.SaveInferred(TStr::Fmt("%s.txt", OutFNm.CStr()));
  nodeSoftMixCascades.SaveWeights(TStr::Fmt("%s_Weights.txt", OutFNm.CStr()));
