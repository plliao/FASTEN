...
int main(int argc, char* argv[]) {
  ...
  const TInt rNNumber = Env.GetIfArgPrefixInt("-rn:", 1,"Maximun number of representative nodes\n");
  const TFlt entropyThreshold = Env.GetIfArgPrefixFlt("-et:", 0.9, "Entropy threshold for selecting representative nodes\n");
  ...
  rNCascade.SetDelta(Delta);
  rNCascade.SetK(k);
  rNCascade.SetGamma(Gamma);
  rNCascade.SetTolerance(Tol);
  rNCascade.SetMaxAlpha(MaxAlpha);
  rNCascade.SetMinAlpha(MinAlpha);
  rNCascade.SetInitAlpha(InitAlpha);
  rNCascade.SetRegularizer(Regularizer);
  rNCascade.SetMu(Mu);
  rNCascade.SetWindow(Window);
  rNCascade.SetAging(Aging);
  rNCascade.SetEntropyThreshold(entropyThreshold);
  rNCascade.SetRNNumber(rNNumber);
  ...
  if (PlotPrecisionRecall || PlotAccuracy || PlotMAE || PlotMSE) {
    ...
    for (THash<TInt, TNodeInfo>::TIter NI = rNCascade.NodeNmH.BegI(); NI<rNCascade.NodeNmH.EndI(); NI++) { rNCascade.Network.AddNode(NI.GetKey()); }
  }
  ...
  if (MaxTimeStr.EqI("-1")) {
    ...
    for (int i=0; i<rNCascade.GetCascs(); i++) {
      if (rNCascade.CascH[i].GetMaxTm() > MaxTime) {
        MaxTime = rNCascade.CascH[i].GetMaxTm();
      }
    }
  } else {
  ...
  if (MinTimeStr.EqI("-1")) {
    ...
    MinTime = TFlt::Mx;
    for (int i=0; i<rNCascade.GetCascs(); i++) {
      if (rNCascade.CascH[i].GetMinTm() < MinTime && rNCascade.CascH[i].GetMinTm()!=0) {
        MinTime = rNCascade.CascH[i].GetMinTm();
      }
    }
  } else {
  ...
  switch (RunningMode) {
      ...
      case INFECTION_STEP:
        ...
        for (int i=0; i<rNCascade.GetCascs(); i++) {
          for (int j=0; j<rNCascade.CascH[i].Len() && rNCascade.CascH[i].NIdHitH[j].Tm < MaxTime; j++) {
            InfectionsV.Add(rNCascade.CascH[i].NIdHitH[j].Tm);
          }
        }
      ...
      case CASCADE_STEP:
        for (int i=0; i<rNCascade.GetCascs(); i++) {
          if (rNCascade.CascH[i].GetMaxTm()<MinTime+MaxTime) { Steps.Add(rNCascade.CascH[i].GetMaxTm()); }
        }
  ...
  rNCascade.Init(Steps);

  TIntV NIds; TStrV NIdsStr;
  if (NodeIdx.EqI("-1")) { rNCascade.Network.GetNIdV(NIds); }
  else {
    NodeIdx.SplitOnAllCh('-', NIdsStr);
    for (int i=NIdsStr[0].GetInt(); i<TInt::GetMn(NIdsStr[1].GetInt(), rNCascade.NodeNmH.Len()); i++) {
      NIds.Add(rNCascade.NodeNmH.GetKey(i));
    }
  }

  for (int i=0; i<NIds.Len(); i++) {
    TStrFltFltHNEDNet::TNodeI NI = rNCascade.Network.GetNI(NIds[i]);
    switch (TOpt) {
        // stochastic gradient
        case OSG:
          rNCascade.SG(NI.GetId(), Iters, Steps, UNIF_SAMPLING, ParamSampling, PlotPerformance);
          break;
      // windowed stochastic gradient
      case OWSG:
          rNCascade.SG(NI.GetId(), Iters, Steps, WIN_SAMPLING, ParamSampling, PlotPerformance);
          break;
      // exponential decay stochastic gradient
      case OESG:
        rNCascade.SG(NI.GetId(), Iters, Steps, EXP_SAMPLING, ParamSampling, PlotPerformance);
          break;
          // exponential decay stochastic gradient
      case OWESG:
        rNCascade.SG(NI.GetId(), Iters, Steps, WIN_EXP_SAMPLING, ParamSampling, PlotPerformance);
        break;
      // rayleigh decay stochastic gradient
      case ORSG:
        rNCascade.SG(NI.GetId(), Iters, Steps, RAY_SAMPLING, ParamSampling, PlotPerformance);
        break;
          // no decay batch stochastic gradient
        case OBSG:
          rNCascade.BSG(NI.GetId(), Iters, Steps, BatchLen, UNIF_SAMPLING, ParamSampling, PlotPerformance);
          break;
        // windowed stochastic gradient
      case OWBSG:
        rNCascade.BSG(NI.GetId(), Iters, Steps, BatchLen, WIN_SAMPLING, ParamSampling, PlotPerformance);
        break;
        // exponential decay batch stochastic gradient
        case OEBSG:
          rNCascade.BSG(NI.GetId(), Iters, Steps, BatchLen, EXP_SAMPLING, ParamSampling, PlotPerformance);
          break;
      // exponential decay batch stochastic gradient
      case ORBSG:
          rNCascade.BSG(NI.GetId(), Iters, Steps, BatchLen, RAY_SAMPLING, ParamSampling, PlotPerformance);
          break;
        // full gradient
        case OFG:
          rNCascade.FG(NI.GetId(), Iters, Steps);
          break;
        default :
          FailR("Bad -s: parameter.");
    }
  ...
  if (SaveOnlyEdges) {
    if (NodeIdx.EqI("-1")) {
      rNCascade.SaveInferredEdges(TStr::Fmt("%s.txt", OutFNm.CStr()));
    } else {
      rNCascade.SaveInferredEdges(TStr::Fmt("%s-%s-%s.txt", OutFNm.CStr(), NIdsStr[0].CStr(), NIdsStr[1].CStr()));
    }
  } else {
    if (NodeIdx.EqI("-1")) {
  ...
  if (PlotPrecisionRecall) {
    TGnuPlot::PlotValV(rNCascade.PrecisionRecall, TStr::Fmt("%s-precision-recall", OutFNm.CStr()), "Precision Recall", "Recall",
                 "Precision", gpsAuto, false, gpwLinesPoints);
  }

  // plot accuracy
  if (PlotAccuracy) {
      TGnuPlot::PlotValV(rNCascade.Accuracy, TStr::Fmt("%s-accuracy", OutFNm.CStr()), "Accuracy", "Time", "Accuracy");
  }

  // Plot MAE
  if (PlotMAE) {
    TGnuPlot::PlotValV(rNCascade.MAE, TStr::Fmt("%s-mae", OutFNm.CStr()), "Normalized MAE", "Time", "Normalized MAE");
  }

  // Plot MSE
  if (PlotMSE) {
    TGnuPlot::PlotValV(rNCascade.MSE, TStr::Fmt("%s-mse", OutFNm.CStr()), "MSE", "Time", "MSE");
  }
