...
int main(int argc, char* argv[]) {
  ...
  // Generate network
  if (TNetwork<2) {
	  softMixCascades.GenerateGroundTruth(TNetwork, NNodes, NEdges, NetworkParams); // Generate network
  } else {
	  TFIn GFIn(GroundTruthFileName);     // open network file
          InfoPathFileIO::LoadNetworkTxt(GFIn, softMixCascades.Network, softMixCascades.nodeInfo);
  }

  // Generate Cascades
  for (int i = 0; i < NCascades; i++) {
	  TCascade C(softMixCascades.CascH.Len(), softMixCascades.nodeInfo.Model);
	  softMixCascades.GenCascade(C);
          softMixCascades.CascH.AddDat(C.CId) = C;
  ...
  if (TNetwork<2) softMixCascades.SaveGroundTruth(FileName);
  InfoPathFileIO::SaveNetwork(TStr::Fmt("%s-network.txt", FileName.CStr()), softMixCascades.Network, softMixCascades.nodeInfo, softMixCascades.edgeInfo);
  // Save Cascades
  InfoPathFileIO::SaveCascades(TStr::Fmt("%s-cascades.txt", FileName.CStr()), softMixCascades.CascH, softMixCascades.nodeInfo);
  softMixCascades.SaveWeights(TStr::Fmt("%s_Weights.txt",FileName.CStr()));
